...
Процедура ЗакрытиеЧекаККМ(Товары, ПризнакВозврата, СуммаНал, СуммаБезнал,
	...
	ОбщегоНазначения.ЗаполнитьШапкуДокумента(ДокЧек, глЗначениеПеременной("глТекущийПользователь"), мВалютаРеглУчета, "Продажа");

	//ДокЧек.Дата           = РабочаяДата;
	ДокЧек.Дата           = ПолучитьСерверFrontOffice().ТекущаяКассоваяДата();
