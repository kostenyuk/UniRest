...
Процедура УстановитьПараметрыФормата(СписокФорматов, ФорматСсылка, Картинка = Неопределено, ИспользоватьПоУмолчанию = Неопределено) Экспорт
	...
	КонецЕсли;
КонецПроцедуры // УстановитьПараметрыФормата()
...
Процедура ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, Настройки) Экспорт
	...
	ИОМД = __ОбщегоНазначенияСервер.ИдентификаторОбъектаМетаданных(Настройки.ОсновнойТип);
	...
	КонецЕсли;
КонецПроцедуры // ДобавитьЭлементВТаблицуТиповПолучателей()
...
КонецПроцедуры // ВыполнитьРассылкиВФоновомЗадании()
...
// 
// Параметры:
//   ПараметрыОтчета (Структура) Отчет, настройки и результат его инциализации.
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Иначе
		...
		Попытка
			СтандартныеПодсистемыПереопределяемый.РассылкаОтчетовПодключитьДопОтчет(ПараметрыОтчета.Отчет, ПараметрыОтчета, Результат);
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс Тогда
		...
		Попытка
			Прокси = Неопределено;
			СтандартныеПодсистемыПереопределяемый.РассылкаОтчетовНастройкиПрокси(Прокси);
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Попытка
			СтандартныеПодсистемыПереопределяемый.РассылкаОтчетовДоставитьВПапку(ПараметрыДоставки, Вложения);
			Результат = Истина;
...
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Иначе
			ПравоДоступа = Истина;
			//СтандартныеПодсистемыПереопределяемый.РассылкаОтчетовПравоИзмененияПапкиИФайлов(ПараметрыДоставки.Папка, ПравоДоступа);
