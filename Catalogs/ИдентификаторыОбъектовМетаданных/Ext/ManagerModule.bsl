...
Процедура ОбновитьДанныеСправочника(ЕстьИзменения = Ложь, ЕстьУдаленные = Ложь) Экспорт
	...
	СправочникМенеджер = __ОбщегоНазначенияСервер.МенеджерОбъектаПоПолномуИмени("Справочник.ИдентификаторыОбъектовМетаданных");
	...
	Пока Чтение.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Если СвойстваИдентификатора = Неопределено
		 ИЛИ СвойстваИдентификатора.ПометкаУдаления Тогда
			...
			Если ОбъектМетаданных <> Неопределено Тогда
				Идентификатор = __ОбщегоНазначенияСервер.ИдентификаторОбъектаМетаданных(ОбъектМетаданных);
...
Процедура ОбновитьСвойстваИдентификатора(Объект) Экспорт
	...
	Если НЕ Объект.ЭтоНовый() Тогда
		СтарыеЗначения = __ОбщегоНазначенияСервер.ПолучитьЗначенияРеквизитов(
			Объект.Ссылка,
			"Наименование,
			|ПорядокКоллекции,
			|Имя,
...
Функция КлючОбъектаМетаданных(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = ВРег("ЖурналДокументов") Тогда
		Возврат ТипЗнч(__ОбщегоНазначенияСервер.МенеджерОбъектаПоПолномуИмени(ПолноеИмя));
	...
	Для каждого ОбъектМетаданных Из КолекцияОбъектовМетаданных Цикл
		...
		Если НЕ СвойстваКоллекции.БезДанных
		   И Найти(СвойстваКоллекции.ИмяВЕдЧисле, "Регистр") = 0 Тогда
			...
			ЗначениеПустойСсылки = __ОбщегоНазначенияСервер.МенеджерОбъектаПоПолномуИмени(ПолноеИмя).ПустаяСсылка();
		Иначе
