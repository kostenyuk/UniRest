...
Процедура ПередПроверкойИдентификаторовОбъектовМетаданныхВПодчиненномУзлеРИБ(Отказ = Ложь) Экспорт
	...
	Если ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить(
			"ИдентификаторыОбъектовМетаданныхВПодчиненномУзлеУжеЗагружены") <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗагрузитьСообщениеОбменаДанными()
	   И НЕ ОбменДаннымиВызовСервера.ПовторитьЗагрузкуСообщенияОбменаДаннымиПередЗапуском() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьСинхронизациюПриОтсутствииОбновленияИнформационнойБазы() Экспорт
	...
	Если ЗагрузитьСообщениеОбменаДанными() Тогда
		...
		// Если выполнен возврат к конфигурации базы данных,
		// тогда загрузка и выгрузка не требуется.
		Если НЕ КонфигурацияИзменена()
		   И НЕ ОбменДаннымиВызовСервера.ПовторитьЗагрузкуСообщенияОбменаДаннымиПередЗапуском() Тогда
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("ОбменДаннымиПравилаИсточников",				  "ОбменДаннымиВызовСервера.УстановкаПараметровСеанса");
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	//ПроверитьВозможностьВыполненияОбменов(); //Костенюк Александр-Закомментировано 09.04.2014
...
Процедура ВыполнитьОбменДаннымиПоРегламентномуЗаданию(КодСценарияОбмена) Экспорт
	...
	//Костенюк Александр-Старт 09.04.2014
	// Если пользователь не указан, значит это фоновое задание
	// Нет необходимости проверять возможность выполнения обменов
	Если НЕ ПустаяСтрока(ИмяПользователя()) Тогда
		ПроверитьВозможностьВыполненияОбменов();
	КонецЕсли;
	УстановитьПривилегированныйРежим(Истина);
	//Костенюк Александр-Финиш 09.04.2014
	...
	УстановитьПривилегированныйРежим(Ложь); //Костенюк Александр 09.04.2014
...
// Для внутреннего использования
// 
Процедура ВыполнитьОбменДаннымиЦРСДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(УзелИнформационнойБазы = Неопределено,
																			ПолноеИмяФайлаСообщенияОбмена = "",
																			ДействиеПриОбмене,
																			ИмяПланаОбмена = "",
																			КодУзлаИнформационнойБазы = "",
...
Процедура ПослеЧтенияСообщенияОбмена(Знач Получатель, Знач СообщениеОбмена, Знач СообщениеПрочитано, СтандартнаяОбработка, Знач УдалитьСообщение = Истина)
	...
	Если ЭтоПодчиненныйУзелРИБ()
		...
		Иначе
			...
			Если УдалитьСообщение Тогда
				...
				Если ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы() Тогда
					// После выгрузки сообщения обмена повторение загрузки отключается.
					// Однако, если в процессе обновления происходит ошибка, следует
					// перейти в режим повторной загрузки, чтобы был выбор - загрузить
					// новое сообщение или продолжить без загрузки.
					ВключитьПовторениеЗагрузкиСообщенияОбменаДаннымиПередЗапуском();
				КонецЕсли;
		...
		Если СохранитьСообщениеВКэш Тогда
			СтароеСообщение = Константы.СообщениеОбменаДаннымиИзГлавногоУзла.Получить().Получить();
			НовоеСообщение  = Новый ДвоичныеДанные(СообщениеОбмена);
			ОбновитьКэш = Ложь;
			
			Если ТипЗнч(СтароеСообщение) <> Тип("ДвоичныеДанные") Тогда
				ОбновитьКэш = Истина;
			Иначе
			...
			Если ОбновитьКэш Тогда
				Константы.СообщениеОбменаДаннымиИзГлавногоУзла.Установить(
					Новый ХранилищеЗначения(НовоеСообщение, Новый СжатиеДанных(9)));
				
				ЗаписатьСобытиеПолученияДанных(
					Получатель, НСтр("ru = 'Сообщение обмена записано в кэш.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаписатьСообщениеСИзменениямиДляУзла(СтруктураНастроекОбмена, Знач ИмяФайлаСообщенияОбмена = "", СообщениеОбмена = "")
	...
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда // Обмен в РИБ
		...
		//.. Начало изменения Dim)on  11 августа 2013 г. 0:42:32
		//
		//ОбработкаОбменаДанными.ВыполнитьВыгрузкуДанныхЦРС_WS(Отказ);
		//
		//Если Не СтруктураНастроекОбмена.ИспользоватьПередачуБольшогоОбъемаДанных Тогда
		//	СообщениеОбмена = ОбработкаОбменаДанными.СообщениеОбмена;
		//КонецЕсли;
		//
		//.. Конец изменения Dim)on  11 августа 2013 г. 0:42:32
...
Процедура ПрочитатьСообщениеСИзменениямиДляУзла(СтруктураНастроекОбмена, Знач ИмяФайлаСообщенияОбмена = "", СообщениеОбмена = "", Знач ТолькоПараметры = Ложь)
	...
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда // Обмен в РИБ
		...
		//.. Начало изменения Dim)on  11 августа 2013 г. 0:43:59
		//
		//ОбработкаОбменаДанными.СообщениеОбмена = СообщениеОбмена;
		//
		//.. Конец изменения Dim)on  11 августа 2013 г. 0:43:59
...
Функция ТаблицаМонитораОбменаДанными(Знач ПланыОбмена, Знач ДополнительныеСвойстваПланаОбмена = "", Знач ТолькоОшибочные = Ложь) Экспорт
	...
	НастройкиСинхронизации.Колонки.Добавить("ПредставлениеРасписания");
	...
	СоответствиеУзелСценарий = Новый Соответствие;
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		
		СоответствиеУзелСценарий.Вставить(ВыборкаДетальныеЗаписи.УзелИнформационнойБазы, ВыборкаДетальныеЗаписи.Ссылка);
	КонецЦикла;
	...
	Для Каждого НастройкаСинхронизации Из НастройкиСинхронизации Цикл
		...
		Сценарий = СоответствиеУзелСценарий.Получить(НастройкаСинхронизации.УзелИнформационнойБазы);
		
		Если Сценарий = Неопределено Тогда
			ПредставлениеРасписания = НСтр("ru = 'не задано'");
		Иначе
			РасписаниеРегламентногоЗадания = Справочники.СценарииОбменовДанными.ПолучитьРасписаниеВыполненияОбменаДанными(Сценарий);
		...
		НастройкаСинхронизации.ПредставлениеРасписания = ПредставлениеРасписания;
	...
	Если Контекст <> Неопределено Тогда
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			Если Контекст.Свойство(Реквизит.Имя) Тогда
				...
				//.. Начало изменения Dim)on  27 июля 2013 г. 16:36:20
				//
				Если ТипЗнч(Форма[Реквизит.Имя]) = Тип("ДанныеФормыДерево") Тогда
					
					Элементы = Контекст[Реквизит.Имя].ПолучитьЭлементы();
					
					Если Элементы.Количество() = 0 Тогда
						
						Форма[Реквизит.Имя].ПолучитьЭлементы().Очистить();
						
					Иначе
						
						//ЭлементыНаФорме = Форма[Реквизит.Имя].ПолучитьЭлементы();
						//
						//Таблица = Новый ТаблицаЗначений;
						//
				...
				КонецЕсли;
				//
				//.. Конец изменения Dim)on  27 июля 2013 г. 16:36:20
...
// Для внутреннего использования
// 
Функция ПолучитьWSПрокси(СтруктураНастроек, СтрокаСообщенияОбОшибке = "", СообщениеПользователю = "", Таймаут = 60) Экспорт
	
	УдалитьНезначащиеСимволыВНастройкахПодключения(СтруктураНастроек);
	
	WSURLПространстваИменСервиса = Неопределено;
	Если Не СтруктураНастроек.Свойство("WSURLПространстваИменСервиса") Или WSURLПространстваИменСервиса = Неопределено Или ПустаяСтрока(WSURLПространстваИменСервиса) Тогда
		СтруктураНастроек.Вставить("WSURLПространстваИменСервиса", "Exchange2K_CRS");
	КонецЕсли;
	WSИмяСервиса = Неопределено;
	Если Не СтруктураНастроек.Свойство("WSИмяСервиса", WSИмяСервиса) Или WSИмяСервиса = Неопределено Или ПустаяСтрока(WSИмяСервиса) Тогда
		СтруктураНастроек.Вставить("WSИмяСервиса", "Exchange2K_CRS");
	КонецЕсли;
...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент, Знач СтруктураНастроек, СообщениеПользователю = "") Экспорт
	...
	WSПрокси = ПолучитьWSПрокси(СтруктураНастроек,, СообщениеПользователю, 5);
...
Функция СоздатьВременныйКаталогСообщенийОбмена() Экспорт
	...
	Если Не __ОбщегоНазначенияСервер.ИнформационнаяБазаФайловая() Тогда
...
// Получает структуру настроек обменов ЦРС из информационной базы
//
// Параметры:
//  Нет.
// 
// Возвращаемое значение:
//  ОбменДаннымиПравилаИсточников - Структура
// 
Функция ПолучитьПравилаРегистрацииОбъектовЦРС() Экспорт 

	// возвращаемое значение функции
	ПравилаРегистрацииОбъектовЦРС = ИнициализацияСтруктурыПравилРегистрацииОбъектовЦРС();
	
	ПравилаРегистрацииОбъектовЦРС.Вставить("ЭтотУзел", ПланыОбмена.ОбменДанными2K_CRS.ЭтотУзел());
	ПравилаРегистрацииОбъектовЦРС.Вставить("ЭтоЦБ", ПроцедурыОбменаДаннымиFrontOffice_Restoran.ЭтоЦентральнаяБаза(ПравилаРегистрацииОбъектовЦРС.ЭтотУзел));
	
	ПравилаРегистрацииОбъектовЦРС.Вставить("ТаблицаИерархииИдентификаторов", ПланыОбмена.ОбменДанными2K_CRS.ПолучитьТаблицуИерархииИдентификаторов());
	ПравилаРегистрацииОбъектовЦРС.Вставить("СортировкаДереваИерархииИдентификаторов", ПланыОбмена.ОбменДанными2K_CRS.ПолучитьТаблицуСортировкиДерева());
	//
	ПравилаРегистрацииОбъектовЦРС.Вставить("СоответствиеСписковИменРеквизитовКОбработкеПриОбмене", );
	СтруктураСоставаПланаОбменаПоПолномуИмени = Неопределено;
	ПравилаРегистрацииОбъектовЦРС.Вставить("СписокСоставаПланаОбменаПоПолномуИмени", ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьСписокОбъектовСоставаПланаОбмена("ОбменДанными2K_CRS", Истина, СтруктураСоставаПланаОбменаПоПолномуИмени));
	//ПравилаРегистрацииОбъектовЦРС.Вставить("СтруктураСоставаПланаОбменаПоПолномуИмени", СтруктураСоставаПланаОбменаПоПолномуИмени);
	ПравилаРегистрацииОбъектовЦРС.Вставить("ИспользованиеРеквизитаУзелИсточникПоПолномуИмени", ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьСписокОбъектовСРеквизитомУзелИсточник());
	ПравилаРегистрацииОбъектовЦРС.Вставить("ТаблицаНастроекРИБ", ПроцедурыОбменаДаннымиFrontOffice_Restoran.СформироватьТаблицуНастроекРИБ());
	ПравилаРегистрацииОбъектовЦРС.Вставить("ТаблицаНастроекРИББух", ПроцедурыОбменаДаннымиFrontOffice_Restoran.СформироватьТаблицуНастроекРИББух());
	
	Возврат ПравилаРегистрацииОбъектовЦРС;	

КонецФункции // ПолучитьПравилаРегистрацииОбъектовЦРС()
...
// Для внутреннего использования
// 
Функция ИнициализацияСтруктурыПравилРегистрацииОбъектовЦРС() Экспорт
