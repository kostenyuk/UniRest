...
Функция ПолучитьОбщиеДанныеУзлов(ИмяПланаОбмена, КодПодчиненного = "")
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если ИмяПланаОбмена = "ОбменДанными2K_CRS" Тогда
		
		Запись = Новый ЗаписьXML;
		Запись.УстановитьСтроку("UTF-8");
		
		//ЗаписатьДанныеУзловПланаОбмена(Запись, ПланыОбмена.ОбменДанными2K_CRS.НайтиПоКоду(КодПодчиненного));
		//
		...
		|	ОбменДанными2K_CRS.Код
		|ИЗ
		|	ПланОбмена.ОбменДанными2K_CRS КАК ОбменДанными2K_CRS
		|ГДЕ
		|	(ОбменДанными2K_CRS.Код = &КодПодчиненного
		|			ИЛИ ОбменДанными2K_CRS.Ссылка = &ЭтотУзел)";
		
		Запрос.УстановитьПараметр("КодПодчиненного", КодПодчиненного);
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			НастройкиСвязейМеждуБазамиРС = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
			ЗаписатьXML(Запись, НастройкиСвязейМеждуБазамиРС);
		КонецЦикла;
		...
		Возврат СериализаторXDTO.ЗаписатьXDTO(Новый Структура("Ответ", СтрXML))
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления()
	...
	//Если Ложь И ЗначениеЗаполнено(ОбновлениеИнформационнойБазы.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
	//	
...
Процедура ЗаписатьДанныеУзловПланаОбмена(Знач ЗаписьСообщения, Получатель)
	
	Если ПланыОбмена.ГлавныйУзел() = Неопределено Тогда
		
		мУзлов = Новый Массив;
		мУзлов.Добавить(Получатель);
		...
		Для каждого Узел Из мУзлов Цикл
			...
			Фабрика.ЗаписатьXML(ЗаписьСообщения, УзелПланаОбмена);
