...
Процедура ТаблицаОткрытыхДокументовПрочитать(ТабличноеПоле, ТребуемыеПолномочия = "ПросмотрСпискаОткрытых", СписокИсключаемых = Неопределено, Реализация = Истина, Списание = Истина) Экспорт
	...
	Если (Данные = Неопределено) Тогда
		// Александр Переверзев 11.03.2012 21:52:09 
		...
		// Александр Переверзев 11.03.2012 21:52:59 
		...
		// Александр Переверзев 11.03.2012 21:52:55 
	...
	// Александр Переверзев 03.03.2012 22:28:53 \
	...
	// Чистяков Павел 09.12.2011 11:02:05 
	...
	//Костенюк Александр-Старт 31.07.12
	|	ВЫБОР
	...
	| 	КОНЕЦ КАК Комната,
    //Костенюк Александр-Финиш 31.07.12
	...
	Если ЗначениеЗаполнено(ТекущийСотрудник)
		//И (Данные.Найти(ТекущийСотрудник, "Сотрудник") = Неопределено)
		//// Чистяков Павел 09.12.2011 11:02:05 
		//И ДоступностьЗаказовСоздание Тогда
...
Процедура ТаблицаОткрытыхДокументовВывестиСтроку(Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт
	...
	// Александр Переверзев 03.03.2012 22:31:09 
	...
	Если Булево(ОформлениеСтроки.Ячейки("Документы").Ячейки(Неопределено)) Тогда
		Для ИндексВложенныхДанных = 0 По ОформлениеСтроки.Ячейки("Документы").Ячейки(Неопределено) - 1 Цикл
			...
			Если (ТипЗнч(ДанныеДокумента.Ссылка) = мТипДокументаРеализацияТоваровУслуг) Тогда
				...
				Если ПустаяСтрока(ДанныеДокумента.Комментарий) Тогда
					//ВложеннаяЯчейка.Текст = ВложеннаяЯчейка.Текст + Символы.ПС + Символы.ПС;
				Иначе
					ВложеннаяЯчейка.Текст = ВложеннаяЯчейка.Текст + Символы.ПС + ДанныеДокумента.Комментарий;
				КонецЕсли;
				
				// Александр Переверзев 03.03.2012 22:30:04 
			...
			КонецЕсли;
			СтруктураСтиля = FrontOffice.ПараметрыСтиляСостоянияДокумента(ДанныеДокумента.Состояние, ,,, Ложь); СтруктураСтиля.Шрифт = Новый Шрифт(ОформлениеСтроки.Шрифт, ,, СтруктураСтиля.Шрифт.Жирный, СтруктураСтиля.Шрифт.Наклонный, СтруктураСтиля.Шрифт.Подчеркивание, СтруктураСтиля.Шрифт.Зачеркивание);
			ЗаполнитьЗначенияСвойств(ВложеннаяЯчейка, СтруктураСтиля);
		КонецЦикла;
	Иначе
		ОформлениеСтроки.Ячейки("Документы").Текст = "Нет документов";
	КонецЕсли;
