...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДаннымиЧерезВебСервисВДвухБазах(Отказ,
	...
	Если Ложь И Не ПустаяСтрока(ПараметрыПодключения.WSURLВебСервиса) Тогда
...
Процедура Инициализация(Узел) Экспорт
	...
	ИспользоватьПараметрыТранспортаHTTP = Ложь;
	...
	//.. Начало изменения Dim)on  11 августа 2013 г. 15:55:08
	//
	ИначеЕсли ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.WS
		Или ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.HTTP Тогда
		
		ИспользоватьПараметрыТранспортаHTTP = Истина;
		
	КонецЕсли;
	//
	//.. Конец изменения Dim)on  11 августа 2013 г. 15:55:31
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле, КодЭтогоУзла, КодНовогоУзла)
	...
	//.. Начало изменения Dim)on  27 июля 2013 г. 19:28:17
	//
	Если НастройкаОтборовНаУзле.Свойство("СтруктураДопРеквизитов") Тогда
		СтруктураДопРеквизитов = НастройкаОтборовНаУзле.СтруктураДопРеквизитов;
	Иначе
		СтруктураДопРеквизитов = Новый Структура;
	КонецЕсли;
	//
	//.. Конец изменения Dim)on  27 июля 2013 г. 19:28:17
	...
	Иначе
		...
		//.. Начало изменения Dim)on  27 июля 2013 г. 19:14:38
		//
		Для каждого ДопРеквизит Из СтруктураДопРеквизитов Цикл
			
			Если ДопРеквизит.Ключ = "НастройкиСвязейМеждуБазамиРС" Тогда
				НовыйУзел[ДопРеквизит.Ключ].Загрузить(ДопРеквизит.Значение)
			Иначе
				НовыйУзел[ДопРеквизит.Ключ] = ДопРеквизит.Значение
			КонецЕсли;
		КонецЦикла;
		//
		//.. Конец изменения Dim)on  27 июля 2013 г. 19:14:38
...
Процедура ВыгрузитьПараметрыМастера(ЗаписьXML)
	...
	//.. Начало изменения Dim)on  11 августа 2013 г. 15:52:30
	//
	Если ИспользоватьПараметрыТранспортаHTTP Тогда
		
		//ЗаписьXML.ЗаписатьНачалоЭлемента("wsПараметры");
		
		ДобавитьЗаписьXML(ЗаписьXML, "WSURLВебСервиса");
		ДобавитьЗаписьXML(ЗаписьXML, "WSЗапомнитьПароль");
		ДобавитьЗаписьXML(ЗаписьXML, "WSИмяПользователя");
		ДобавитьЗаписьXML(ЗаписьXML, "WSПароль");
		
		//ЗаписьXML.ЗаписатьКонецЭлемента(); // wsПараметры
		
	КонецЕсли;
	//
	//.. Конец изменения Dim)on  11 августа 2013 г. 15:52:30
	...
	ДобавитьЗаписьXML(ЗаписьXML, "ИспользоватьПараметрыТранспортаHTTP");
