...
Процедура УстановитьПрефиксИнформационнойБазы(Источник, СтандартнаяОбработка, Префикс) Экспорт
	...
	//Костенюк Александр-Старт 05.09.2014
	// Товары с префиксом CRS-... не закрываются на кассовый аппарат 
	Если ТипЗнч(Источник) = Тип("СправочникОбъект.Номенклатура") Тогда
		Возврат;
	КонецЕсли;
	//Костенюк Александр-Финиш 05.09.2014
...
Процедура УстановитьПрефикс(Источник, Префикс, УстановитьПрефиксИнформационнойБазы, УстановитьПрефиксОрганизации)
	...
	Если УстановитьПрефиксИнформационнойБазы Тогда
		...
		//Костенюк Александр-Старт 22.11.2013
		...
		Если РезультатЗапроса.Пустой() Тогда
			Возврат;
		КонецЕсли;
		ПрефиксИнформационнойБазы = СокрЛП(ЭтотУзел.Код);
		//Костенюк Александр-Финиш 22.11.2013
