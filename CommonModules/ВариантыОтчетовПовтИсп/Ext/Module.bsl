...
Функция ОтчетыКонфигурацииДоступныеТекущемуПользователю() Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		Если ПравоДоступа("Просмотр", ОтчетМетаданные)
			И ВариантыОтчетов.ОбъектМетаданныхДоступенПоФункциональнымОпицям(ОтчетМетаданные) Тогда
			Результат.Добавить(__ОбщегоНазначенияСервер.ИдентификаторОбъектаМетаданных(ОтчетМетаданные));
		КонецЕсли;
	КонецЦикла;
