...
Процедура ОбновитьКэшМеханизмаРегистрацииОбъектов() Экспорт
	...
	Если ПараметрыСеанса.ИспользуемыеПланыОбмена.Количество() > 0 Тогда
		...
		ПараметрыСеанса.ОбменДаннымиПравилаИсточников = Новый ХранилищеЗначения(ОбменДаннымиСервер.ПолучитьПравилаРегистрацииОбъектовЦРС(), Новый СжатиеДанных(0));
	...
	Иначе
		...
		ПараметрыСеанса.ОбменДаннымиПравилаИсточников = Новый ХранилищеЗначения(ОбменДаннымиСервер.ИнициализацияСтруктурыПравилРегистрацииОбъектовЦРС(), Новый СжатиеДанных(0));
...
Процедура УстановкаПараметровСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		УстановленныеПараметры.Добавить("ОбменДаннымиПравилаИсточников");
...
Функция ОбменДаннымиВключен(Знач ИмяПланаОбмена) Экспорт
	...
	// метод нельзя вызывать из модуля, в котором влкючено повторное использование возвращаемых значений
	// т.к. результат выполнения будет один и тот же на время сеанса
	// что является недопустимым, т.к. возможно выделение сразу нескольких ресторанов из ЦРС
	//Возврат (ПараметрыСеанса.ИспользуемыеПланыОбмена.Найти(ИмяПланаОбмена) <> Неопределено) И (ПланыОбмена.ГлавныйУзел() <> Неопределено);
	ОбменВключен = Ложь; //02.09.2014
	Если ПараметрыСеанса.ИспользуемыеПланыОбмена.Найти(ИмяПланаОбмена) <> Неопределено Тогда
		ОбменВключен = Истина;
		ЭтоЦБ = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ЭтоЦентральнаяБаза(ПланыОбмена.ОбменДанными2K_CRS.ЭтотУзел());
		Если НЕ ЭтоЦБ Тогда
			ОбменВключен = ПланыОбмена.ГлавныйУзел() <> Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция ПовторитьЗагрузкуСообщенияОбменаДаннымиПередЗапуском() Экспорт
	...
	Возврат Константы.ПовторитьЗагрузкуСообщенияОбменаДаннымиПередЗапуском.Получить() = Истина
	   И ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить("ПропуститьПовторнуюЗагрузкуСообщенияОбменаДанными") = Неопределено;
	
КонецФункции

// Сбрасывает признак повторения загрузки, установленный при ошибке.
