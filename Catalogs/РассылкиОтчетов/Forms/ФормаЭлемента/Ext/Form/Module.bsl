...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Автор.Доступность = Пользователи.ПолноправныйПользователь();
...
Процедура ПапкаОткрытие(Элемент, СтандартнаяОбработка)
	СтандартныеПодсистемыКлиентПереопределяемый.РассылкаОтчетовПросмотрПапки(СтандартнаяОбработка, Объект.Папка);
КонецПроцедуры
...
Процедура ПредпросмотрШаблона(Команда)
	...
	Настройки = Новый Структура("НаименованиеРассылки, Автор, ЗаголовокСистемы, ДатаВыполнения, СформированныеОтчеты, СпособДоставки", 
		...
		__ОбщегоНазначенияКлиентСервер.ДатаСеанса(),
		СписокОтчетов,
		""
	);
...
&НаСервере
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки)
	// Параметры журнала
...
Функция ПравоИзмененияПапкиИФайлов(Папка)
	...
	Результат = Истина;
	//СтандартныеПодсистемыПереопределяемый.РассылкаОтчетовПравоИзмененияПапкиИФайлов(Папка, Результат);
	Возврат Результат;
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		РезультатВыбора = ВыбратьПолучателя(Объект, Ложь, Истина); // Получатель и E-Mail
		Если РезультатВыбора = Неопределено Тогда
			Возврат;
		КонецЕсли;
