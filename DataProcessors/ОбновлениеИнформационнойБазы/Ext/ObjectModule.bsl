...
Процедура ВыполнитьОбновление() Экспорт
	...
	Если (ТекущаяВерсияИБ = "3.1.1.1")
		...
		ИЛИ ((ОбщегоНазначения.ПолучитьНомерРелиза(ТекущаяВерсияИБ) = "3.1.5") И ТекущаяВерсияИБ <> НоваяВерсияИБ)
		Тогда
		...
		ЗаписьЖурналаРегистрации("ИнформационнаяБаза.ОбновлениеИБ", УровеньЖурналаРегистрации.Информация, , , НСтр("ru='Выполнено ';uk='Виконано '") + СтрСообщения);
