...
// 
Процедура ОткрытьПомощникНастройкиОбменаДанными(Знач ИмяПланаОбмена, НастройкаОбменаССервисом = Ложь, Модально = Ложь, РезультатМодальногоОкна = Неопределено) Экспорт
...
//
Процедура УдалитьНастройкуСинхронизации(Знач УзелИнформационнойБазы, ГрупповоеУдаление = Ложь) Экспорт
	
	Если ГрупповоеУдаление Или 
		Вопрос(НСтр("ru = 'Удалить настройку синхронизации данных с "+УзелИнформационнойБазы+"?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да) = КодВозвратаДиалога.Да Тогда
...
Процедура ПриВыполненииОбновленияИнформационнойБазы(СтандартнаяОбработка, ЗавершениеРаботыСистемы = Ложь)
	...
	ОбновлениеКонфигурацииКлиент.ПриВыполненииОбновленияИнформационнойБазы(СтандартнаяОбработка, ЗавершениеРаботыСистемы);
...
Процедура ПриЗавершенииРаботыПомощникаСозданияОбмена(Помощник, ЗапуститьМенеджерЗаданий) Экспорт
	...
	Если ЗапуститьМенеджерЗаданий Тогда
		//.. Начало изменения Dim)on  27 июля 2013 г. 17:28:42
		//
		//Почему то не видит отсюда модуль   РегламентныеЗаданияСлужебныйГлобальный
		//
		//.. Конец изменения Dim)on  27 июля 2013 г. 17:28:42
		//РегламентныеЗаданияСлужебныйГлобальный.ЗапуститьОтдельныйСеансДляВыполненияРегламентныхЗаданийЧерезОбработчикОжидания();
	КонецЕсли;
...
Процедура ОбновитьКонфигурацию() Экспорт 
	//Отключим обработчики запущенные при старте системой
	//Попытка
		глНеобходимоЗакрытьПрограмму = Истина;

	//Исключение
	//	//ничего не делаем
	//КонецПопытки;
	
	//ОбновлениеКонфигурацииМодульИБ();
	ПодключитьОбработчикОжидания("ОбновлениеКонфигурацииИБ", 0.1, Истина);
	
КонецПроцедуры

Процедура ОбновлениеКонфигурацииМодульИБ() Экспорт
	
	ОтключитьОбработчикОжидания("ОбновлениеКонфигурацииИБ");
	
	МассивСоединений = ПолучитьСоединенияИнформационнойБазы();
	Если МассивСоединений.Количество() > 1 Тогда
		Сообщить("Открыто несколько программ 1С Предприятия, необходимо закрыть остальные.", СтатусСообщения.Важное);
		ПодключитьОбработчикОжидания("ОбновлениеКонфигурацииИБ", 10, Истина);
		Возврат; // ждем дальше...
	КонецЕсли;
	
	текПерезапуститьПриЗавершении = Истина;
	текЗапрашиватьВозможность = Истина;
	//текЗапрашиватьВозможность = Ложь;
	ЗавершитьРаботуСистемы(текЗапрашиватьВозможность, текПерезапуститьПриЗавершении, " CONFIG /UpdateDBCfg ");
	ПодключитьОбработчикОжидания("ОбновлениеКонфигурацииИБ", 5, Истина);

	////СтрокаСоединения = СтрокаСоединенияИнформационнойБазы();
	//АдресБазы = ПолучитьАдресБазыДляСтрокиПодключения();
	...
	//Команда = """""" + КаталогПрограммы() + "1CV8.EXE"""" CONFIG"
	//	+ АдресБазы + " /N Exchange /P Exchange /UpdateDBCfg";
	...
	//Команда = """""" + КаталогПрограммы() + "1CV8.EXE"""" ENTERPRISE"
	//	+ АдресБазы + " /N Exchange /P Exchange";
