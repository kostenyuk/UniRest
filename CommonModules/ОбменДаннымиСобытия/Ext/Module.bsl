...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	//.. Начало изменения Dim)on  4 августа 2013 г. 22:14:06
	//
		Возврат;
	//
	//.. Конец изменения Dim)on  4 августа 2013 г. 22:14:06
...
Процедура МеханизмРегистрацииОбъектов(ИмяПланаОбмена,
	...
	Попытка
		...
		//Разделение не используем
		Если Ложь И ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		//.. Начало изменения Dim)on  12 августа 2013 г. 11:03:23
		//
		Если ОбъектМодифицирован И (Объект.ОбменДанными.Получатели.Количество() Или Объект.ОбменДанными.Загрузка)
			И (ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных) Или ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных))
			Тогда
		
			ОбменДаннымиЦРАСобытия.УстановитьУзелИсточникДляОбъекта(Объект, ОбъектМетаданных, Отказ);
		
		КонецЕсли;
		//
		//.. Конец изменения Dim)on  12 августа 2013 г. 11:03:23
...
Процедура ВыполнитьПравилаРегистрацииОбъектовДляПланаОбмена(МассивУзловРезультат,
															...
															Выгрузка = Ложь)
	Попытка
		ВыполнитьПравилаРегистрацииОбъектовЦРСДляПланаОбменаПопыткаИсключение(МассивУзловРезультат,
															Объект,
															ИмяПланаОбмена,
															ОбъектМетаданных,
															ПроверятьСсылку,
...
Процедура ВыполнитьПравилаРегистрацииОбъектовЦРСДляПланаОбменаПопыткаИсключение(МассивУзловРезультат,
															Объект,
															ИмяПланаОбмена,
															ОбъектМетаданных,
															ПроверятьСсылку,
	...
	Если Не "ОбменДанными2K_CRS" = ИмяПланаОбмена Тогда
		Возврат	
	КонецЕсли;
	
	ЭтотУзел = ПланыОбмена[ИмяПланаОбмена].ЭтотУзел();
	
	//Костенюк Александр-Старт 15.04.2014
	ЭтоЦБ = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ЭтоЦентральнаяБаза(ЭтотУзел);
	Получатели = Новый Массив;
	//Костенюк Александр-Финиш 15.04.2014
	
	Если ЭтоРегистр Тогда
		//ОбъектМетаданных = Метаданные.РегистрыСведений.АнализДействий
		//Если ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.АнализДействий") Тогда
		//	Получатели = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьМассивУзловДляРегистрацииПоПравиламРесурсов(Объект.Отбор.Заказ.Значение.Метаданные().ПолноеИмя(), ЭтотУзел);
		//ИначеЕсли ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.СомнительныеСитуации") Тогда
		//	Получатели = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьМассивУзловДляРегистрацииПоПравиламРесурсов("Документ.РеализацияТоваровУслуг", ЭтотУзел);
		
		//Костенюк Александр-Старт 29.05.2015
		Если НЕ Объект.Количество() Тогда
			Возврат;
		КонецЕсли;
		//Костенюк Александр-Финиш 29.05.2015
		
		//Костенюк Александр-Старт 11.04.2014
		Если (ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.АнализДействий")) ИЛИ
			(ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.СомнительныеСитуации")) Тогда
			Если ЭтоЦБ Тогда
				////Этот объект относится к базе из которой пришел. 
				////Любые изменения с ним в главной базе должны перейти в базу этого узла и больше соответсвенно ни к кому
				//Если НЕ Объект.ОбменДанными.Загрузка Тогда
				//	Если ЗначениеЗаполнено(Объект.УзелИсточник) И (ЭтотУзел <> Объект.УзелИсточник) Тогда
				//		Получатели.Добавить(Объект.УзелИсточник);
				//	КонецЕсли;
				//КонецЕсли;
			Иначе
				Получатели.Добавить(ПланыОбмена.ГлавныйУзел());
			КонецЕсли;
		//Костенюк Александр-Финиш 11.04.2014
		//Костенюк Александр-Старт 07.08.2014
		ИначеЕсли ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.СинонимыОбъектов") Тогда
			Если (ТипЗнч(Объект.Отбор.Объект.Значение) = Тип("СправочникСсылка.СотрудникиОрганизаций"))
				ИЛИ (ТипЗнч(Объект.Отбор.Объект.Значение) = Тип("СправочникОбъект.СотрудникиОрганизаций")) Тогда
				Возврат;
			КонецЕсли;
			Получатели = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьМассивУзловДляРегистрацииПоПравиламИсточников(ОбъектМетаданных.ПолноеИмя(), ЭтотУзел);
		// Регистрация набора записей регистра "СинонимыОбъектов"
		ИначеЕсли ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.СотрудникиРесторанов") И ЭтоЦБ Тогда
			Узел = ПланыОбмена.ОбменДанными2K_CRS.НайтиПоРеквизиту("Ресторан", Объект.Отбор.Ресторан.Значение);
			ПланыОбмена.ЗарегистрироватьИзменения(Узел, Объект.Отбор.Сотрудник.Значение);
			НаборЗаписей = РегистрыСведений.СинонимыОбъектов.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.ИнформационнаяСистема.Установить(Константы.ОсновнаяИнформационнаяСистема.Получить());
			НаборЗаписей.Отбор.Объект.Установить(Объект.Отбор.Сотрудник.Значение);
			ПланыОбмена.ЗарегистрироватьИзменения(Узел, НаборЗаписей);
		//Костенюк Александр-Финиш 07.08.2014
		Иначе
			Получатели = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьМассивУзловДляРегистрацииПоПравиламИсточников(ОбъектМетаданных.ПолноеИмя(), ЭтотУзел);
			
			Если Получатели.Количество() И ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.ЦеныНоменклатуры") Тогда
				// для цен необходимо проследить за регистрацией по типу цен.
				// в каждый узел должны зарегистрироваться только те цены, которые соответствуют типу цен ресторана узла.
				Если Объект.Отбор.ТипЦены.Использование Тогда
					Получатели = ОбменДаннымиПовтИсп.ПолучитьУзелПоТипуЦенРесторана(Объект.Отбор.ТипЦены.Значение, ЭтотУзел)
				ИначеЕсли Объект.Количество() = 1 Тогда
					Получатели = ОбменДаннымиПовтИсп.ПолучитьУзелПоТипуЦенРесторана(Объект[0].ТипЦены, ЭтотУзел)
				Иначе
					Запрос = Новый Запрос;
					Запрос.Текст = 
					"ВЫБРАТЬ
					...
					|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Объект КАК Объект
					|		ПО ОбменДанными2K_CRS.Ресторан.ТипЦен = Объект.ТипЦены
					|			И (ОбменДанными2K_CRS.Ссылка <> &ЭтотУзел)";
					
					Запрос.УстановитьПараметр("Объект", Объект.Выгрузить());
					...
					Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
						...
						НЗ.Отбор.ТипЦены.Установить(ВыборкаДетальныеЗаписи.ТипЦены);
						ПланыОбмена.ЗарегистрироватьИзменения(ВыборкаДетальныеЗаписи.Узел, НЗ)
					КонецЦикла;
			...
			// Регистрация набора записей регистра "СебестоимостьНоменклатуры" по ресторанам
			Если ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.СебестоимостьНоменклатуры") Тогда
				Если ЭтоЦБ Тогда
					Узел = ПланыОбмена.ОбменДанными2K_CRS.НайтиПоРеквизиту("Ресторан", Объект.Отбор.Ресторан.Значение);
					Получатели.Очистить();
	...
	ИначеЕсли ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных) Тогда
		//Если ЭтотУзел = ПланыОбмена.ГлавныйУзел()
		////Костенюк Александр 07.05.2014
		//// Неверное сравнение с главным узлом
		//// т.к. у центрального узла ПланыОбмена.ГлавныйУзел() = Неопределено
		//	И ЗначениеЗаполнено(Объект.УзелИсточник)
		//	И ЭтотУзел <> Объект.УзелИсточник Тогда
		//	//Этот объект относится к базе из которой пришел. 
		//	//Любые изменения с ним в главной базе должны перейти в базу этого узла и больше соответсвенно ни к кому
		//	Получатели = Новый Массив;
		//	Получатели.Добавить(Объект.УзелИсточник);
		
		//Костенюк Александр-Старт 14.04.2014
		Если ЭтоЦБ Тогда
			Если Объект.Метаданные().Реквизиты.Найти("УзелИсточник") <> Неопределено Тогда
				Если ЗначениеЗаполнено(Объект.УзелИсточник)	И (ЭтотУзел <> Объект.УзелИсточник) Тогда
					Получатели.Добавить(Объект.УзелИсточник);
				КонецЕсли;
			КонецЕсли;
		...
		Если ТипЗнч(Объект) = Тип("СправочникОбъект.СотрудникиОрганизаций") Тогда
			...
			Пока Выборка.Следующий() Цикл
				ПланыОбмена.ЗарегистрироватьИзменения(Выборка.Узел, Выборка.Сотрудник);
			КонецЦикла;
		// Регистрация элемента справочника "Рестораны"
		ИначеЕсли ТипЗнч(Объект) = Тип("СправочникОбъект.Рестораны") Тогда
			Если Объект.Ссылка = Справочники.Рестораны.ПустаяСсылка() Тогда
				Возврат;
			КонецЕсли;
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ
			               |	ОбменДанными2K_CRS.Ссылка КАК Узел,
			               |	ОбменДанными2K_CRS.Ресторан
			               |ИЗ
			               |	ПланОбмена.ОбменДанными2K_CRS КАК ОбменДанными2K_CRS
			               |ГДЕ
			               |	ОбменДанными2K_CRS.Ссылка <> &ЭтотУзел
			               |	И ОбменДанными2K_CRS.Ресторан = &Ресторан";
			Запрос.УстановитьПараметр("ЭтотУзел", ЭтотУзел);
			Запрос.УстановитьПараметр("Ресторан", Объект.Ссылка);
			Выборка = Запрос.Выполнить().Выбрать();
			Пока Выборка.Следующий() Цикл
				ПланыОбмена.ЗарегистрироватьИзменения(Выборка.Узел, Выборка.Ресторан);
			КонецЦикла;
		//Костенюк Александр-Финиш 14.04.2014
		Иначе	
			Получатели = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьМассивУзловДляРегистрацииПоПравиламИсточников(ОбъектМетаданных.ПолноеИмя(), ЭтотУзел);
		КонецЕсли;
	ИначеЕсли ОбщегоНазначения.ЭтоДокумент(ОбъектМетаданных) Тогда
		//Получатели = ПроцедурыОбменаДаннымиFrontOffice_Restoran.ПолучитьМассивУзловДляРегистрацииПоПравиламРесурсов(ОбъектМетаданных.ПолноеИмя(), ЭтотУзел);
		//Костенюк Александр-Старт 07.05.2014
		Если ЭтоЦБ Тогда
			//Этот объект относится к базе из которой пришел. 
			//Любые изменения с ним в главной базе должны перейти в базу этого узла и больше соответсвенно ни к кому
			Если ЗначениеЗаполнено(Объект.УзелИсточник)	И (ЭтотУзел <> Объект.УзелИсточник) Тогда
		...
		Иначе
			Получатели.Добавить(ПланыОбмена.ГлавныйУзел());
		КонецЕсли;
		//Костенюк Александр-Финиш 07.05.2014
