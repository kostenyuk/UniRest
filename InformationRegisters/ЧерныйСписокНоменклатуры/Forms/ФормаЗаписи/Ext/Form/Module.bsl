...
Процедура КоличествоПриИзменении(Элемент)
	...
	Если (ИмяФормы = "РегистрСведений.ЧерныйСписокНоменклатуры.Форма.ФормаУпрощенная") Тогда
		Актуальность = Истина;
	Иначе
...
Процедура ОКСервер()
	...
	Если (ТаблицаВхождений.Количество() = 1) Тогда
		Если УчетКоличества И (ИмяФормы = "РегистрСведений.ЧерныйСписокНоменклатуры.Форма.ФормаУпрощенная") Тогда
			ТаблицаВхождений[0].Актуальность = Актуальность;
			ТаблицаВхождений[0].Количество = Запись.Количество;
		КонецЕсли; 
	КонецЕсли; 
	
    РегистрыСведений.ЧерныйСписокНоменклатуры.УстановитьТаблицуВхожденийНоменклатуры(Запись.Номенклатура, ТаблицаВхождений);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Отказ Тогда
		УчетКоличества = ПолучитьПараметрыУчета().ЧерныйСписокНоменклатурыУчетКоличества;
	КонецЕсли;
...
Процедура ПриОткрытииСервер(Отказ)
	...
	ТаблицаВхождений = РегистрыСведений.ЧерныйСписокНоменклатуры.ПолучитьТаблицуВхожденийНоменклатуры(Запись.Номенклатура);
	
	Если (ТаблицаВхождений.Количество() = 1) Тогда
		Если УчетКоличества И (ИмяФормы = "РегистрСведений.ЧерныйСписокНоменклатуры.Форма.ФормаУпрощенная") Тогда
			Актуальность = ТаблицаВхождений[0].Актуальность;
