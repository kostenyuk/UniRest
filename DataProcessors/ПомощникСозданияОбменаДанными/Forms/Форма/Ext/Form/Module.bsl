...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//.. Начало изменения Dim)on  17 августа 2013 г. 16:01:17
	//
	Если Объект.ИмяПланаОбмена = "Расписание" Тогда
		РежимРаботыМастера = "Расписание";
		ВариантРаботыМастера = "Расписание";
		ЗаполнитьТаблицуПереходов();
		ВыполнятьОбменДаннымиАвтоматически = Истина;
		ПредопределенноеРасписаниеОбменаДанными = "ДругоеРасписание";
		Параметры.Свойство("УзелИнформационнойБазы", Объект.УзелИнформационнойБазы);
		Сценарий = Справочники.СценарииОбменовДанными.ПолучитьСценарий(Объект.УзелИнформационнойБазы);
		Если Не Сценарий.Пустая() Тогда
			...
			РасписаниеВыполненияОбменаДанными = ?(РегламентноеЗаданиеОбъект = Неопределено, Неопределено, РегламентноеЗаданиеОбъект.Расписание);
		КонецЕсли;
		ПредставлениеПользовательскогоОписания = Строка(РасписаниеВыполненияОбменаДанными);
		Элементы.ВыполнятьОбменДаннымиАвтоматически.Видимость = Ложь;
	...
	ВыполнитьСозданиеНачальногоОбразаСейчас = Ложь;
	...
	Если ИспользоватьТранспортСообщенийОбменаWS Или НастройкаОбменаССервисом Тогда
	...
	ЗаголовокФормы = НСтр("ru='Настройка обмена данными %Программа%'");
	...
	Если Не ЗаданоНаименованиеЭтойБазы Тогда
		...
		//Если Объект.ПрефиксИнформационнойБазыИсточника = "ЦБР" Тогда
		//	Объект.НаименованиеЭтойБазы = Объект.НаименованиеЭтойБазы + " (Центральная РС)"	
		//КонецЕсли;
	...
	//.. Начало изменения Dim)on  17 августа 2013 г. 16:01:17
	//
	Если Объект.ИмяПланаОбмена = "ОбменДанными2K_CRSБухгалтерия" Тогда
		ВыборкаПланыОбмена = МенеджерПланаОбмена.Выбрать();
		Пока ВыборкаПланыОбмена.Следующий() Цикл
			Если Не (ВыборкаПланыОбмена.ПометкаУдаления Или ВыборкаПланыОбмена.Ссылка = МенеджерПланаОбмена.ЭтотУзел()) Тогда
				УзелИнформационнойБазы = ВыборкаПланыОбмена.Ссылка			
			КонецЕсли;
		КонецЦикла;
		ВариантРаботыМастера = ?(ЗначениеЗаполнено(УзелИнформационнойБазы), "ПродолжитьНастройкуОбменаДанными", "НастроитьНовыйОбменДанными");		
	    РежимРаботыМастера = "ОбменЧерезОбычныеКаналыСвязиСБухгалтерией";
		
		Объект.ПрефиксИнформационнойБазыИсточника = "БР";
		Объект.ПрефиксИнформационнойБазыПриемника = "БК";
		Объект.ПрефиксИнформационнойБазыПриемникаУстановлен = Истина;
		Объект.НаименованиеВторойБазы = "2К:Бухгалтерия";
		ЗаданоНаименованиеВторойБазы = Не ПустаяСтрока(Объект.НаименованиеВторойБазы);
		
		ЗаполнитьТаблицуПереходов();
		ОбновитьНадписиЗаголовковИнформационныхБаз();
        ПриСозданииНаСервереОбменСБухгалтерией(Отказ, СтандартнаяОбработка);
		
	КонецЕсли;
	//
	//.. Конец изменения Dim)on  17 августа 2013 г. 16:01:17
	...
	Объект.FILEСжиматьФайлИсходящегоСообщения = Ложь;
	Объект.FTPСжиматьФайлИсходящегоСообщения  = Ложь;
	Элементы.ПарольАрхиваСообщенияОбмена3.Доступность = Объект.FILEСжиматьФайлИсходящегоСообщения;
	Элементы.ПарольАрхиваСообщенияОбмена4.Доступность = Объект.FTPСжиматьФайлИсходящегоСообщения;

КонецПроцедуры

&НаСервере
Процедура ПриСозданииНаСервереОбменСБухгалтерией(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ВидТранспортаСообщенийОбмена) Тогда
		
		ИмяСтраницы = "НастройкиТранспорта[ВидТранспорта]1";
		ИмяСтраницы = СтрЗаменить(ИмяСтраницы, "[ВидТранспорта]"
		, ОбщегоНазначения.ИмяЗначенияПеречисления(Объект.ВидТранспортаСообщенийОбмена));
		...
		ВидТранспортаСообщенийОбменаСБухгалтерией = "СетевойКаталог";
...
Процедура КомандаГотово(Команда)
	...
	ИначеЕсли РежимРаботыМастера = "ОбменЧерезВебСервис" Тогда
		...
		ВыполнятьОбменДаннымиАвтоматически = Не ПроцедурыОбменаДаннымиFrontOffice_Restoran.ЭтоЦентральнаяБаза(ПланыОбмена[Объект.ИмяПланаОбмена].ЭтотУзел());
		Если ВариантРаботыМастера = "ПродолжитьНастройкуОбменаДанными" Тогда
			ЗавершениеВторогоЭтапаНастройкиОбменаЧерезWebСервис(Отказ);
			ВыполнитьСозданиеНачальногоОбразаСейчас = Ложь;
			ВыполнитьРегистрациюДанныхДляПодчиненногоУзла = Ложь;
		КонецЕсли;
		Если Не Отказ Тогда
		...
		Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
			...
			Если ВыполнитьРегистрациюДанныхДляПодчиненногоУзла Тогда
			
				ОбменДаннымиКлиент.ОткрытьСоставОтправляемыхДанных(Объект.УзелИнформационнойБазы);
	...
	ИначеЕсли РежимРаботыМастера = "Расписание" Тогда
...
&НаКлиенте
Процедура ПроверитьПодключениеFILEFTP(Команда)
	ПроверитьПодключение(?(ВидТранспортаСообщенийОбменаСБухгалтерией = "СетевойКаталог", "FILE", "FTP"));
КонецПроцедуры
...
Процедура УстановитьВидимостьНаСервере()
	...
	//Костенюк Александр-Старт 15.05.2014
	// Возможность установки префикса в ручном режиме
	...
	//Костенюк Александр-Финиш 15.05.2014
...
Процедура ПроверитьПодключениеWSНаКлиенте(Отказ, РасширеннаяПроверка = Ложь)
	...
	//.. Начало изменения Dim)on  27 июля 2013 г. 15:37:26
	//
	Если Не ОбменДаннымиСервер.ЭтоПодчиненныйУзелРИБ() Тогда
		// проверить существование веб сервиса, проверить наличие вебсервера, предложить установить апач или IIS, 
		//ОПУБЛИКОВАТЬ ВЕБ СЕРВИС НА СЕРВЕРЕ.
		//Выбрать пользователя для домтупа.
		ПользователиСлужебныйВызовСервера.ПолучитьСлужебногоПользователяДляПодключенияКВебСервису();
	КонецЕсли;
	////
	//.. Конец изменения Dim)on  27 июля 2013 г. 15:37:26
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	Если ВариантРаботыМастера = "НастроитьНовыйОбменДанными" Тогда
		...
		ИначеЕсли РежимРаботыМастера = "ОбменЧерезВебСервис" Тогда
			...
			//.. Начало изменения Dim)on  27 июля 2013 г. 15:56:52
			//
			ПорядковыйНомерПерехода = 1;
			ТаблицаПереходовПростойНастройкиОбменаЧерезВебСервис()
			//
			//.. Конец изменения Dim)on  27 июля 2013 г. 15:56:52
			
		ИначеЕсли РежимРаботыМастера = "ОбменЧерезОбычныеКаналыСвязиСБухгалтерией" Тогда
			
			ТаблицаПереходовПервогоЭтапаНастройкиОбменаСБухгалтерией();
			
		КонецЕсли;
		
	ИначеЕсли ВариантРаботыМастера = "Расписание" Тогда
			
			ТаблицаПереходовНастройкаТолькоРасписания();
			
	Иначе // "ПродолжитьНастройкуОбменаДанными"
		
		//.. Начало изменения Dim)on  11 августа 2013 г. 17:10:35
		//
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ()
			И Константы.НастройкаПодчиненногоУзлаРИБЗавершена.Получить() = Ложь Тогда
			ТаблицаПереходовПродолжениеНастройкиОбменаПодчиненногоССервисомГлавного();
		Иначе
...
Процедура ВариантРаботыМастераПриИзмененииНаСервере()
	...
	Если ВариантРаботыМастера = "ПродолжитьНастройкуОбменаДанными" Тогда
		...
		//.. Начало изменения Dim)on  11 августа 2013 г. 17:11:32
		//
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ()
			И Константы.НастройкаПодчиненногоУзлаРИБЗавершена.Получить() = Ложь Тогда
			РежимРаботыМастера = "ОбменЧерезВебСервис";
		КонецЕсли;
		//.. Конец изменения Dim)on  11 августа 2013 г. 17:11:32
		
	Иначе
		
		Если Истина Или НастройкаОбменаССервисом Тогда
...
Процедура РежимРаботыМастераПриИзмененииНаСервере()
	...
	СтраницыКартинок.Вставить("ОбменЧерезОбычныеКаналыСвязиСБухгалтерией", Элементы.КартинкаДляОбменаЧерезОбычныеКаналыСвязи);
	...
	СтраницыПараметровТранспорта.Вставить("ОбменЧерезОбычныеКаналыСвязиСБухгалтерией", Элементы.СтраницаПараметровТранспорта);
	...
	ВидыТранспортаСообщенийОбмена.Вставить("ОбменЧерезОбычныеКаналыСвязиСБухгалтерией", Перечисления.ВидыТранспортаСообщенийОбмена.FILE);
	...
	//.. Начало изменения Dim)on  27 июля 2013 г. 15:34:31
	//
	Объект.ИспользоватьПараметрыТранспортаHTTP = РежимРаботыМастера = "ОбменЧерезВебСервис";
 	Объект.ИспользоватьПараметрыТранспортаFILE = РежимРаботыМастера = "ОбменЧерезОбычныеКаналыСвязи";
 	Объект.ИспользоватьПараметрыТранспортаCOM = РежимРаботыМастера = "ОбменЧерезВнешнееСоединение";
	Объект.ИспользоватьПараметрыТранспортаFILE = РежимРаботыМастера = "ОбменЧерезОбычныеКаналыСвязи" Или РежимРаботыМастера = "ОбменЧерезОбычныеКаналыСвязиСБухгалтерией";
 	
	УстановитьВидимостьПанелиНастроекВебСервисаНаСервере();
	//
	//.. Конец изменения Dim)on  27 июля 2013 г. 15:34:31
...
&НаКлиенте
Процедура ЗавершениеВторогоЭтапаНастройкиОбменаЧерезWebСервис(Отказ)
...
&НаКлиенте
Процедура ПолучитьВнешнийIPЭтогоСервера()
	
	ВнешнийIP = "localhost";
	ВнешнийПорт = "8080";

	Если ПустаяСтрока(ВнешнийIP) Тогда
		ВнешнийIP = "127.0.0.1" 		
	КонецЕсли;
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая(СтрокаСоединенияИнформационнойБазы) Тогда
		//File="C:\***\***";
		мПодстрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаСоединенияИнформационнойБазы, "\");
		ИмяПубликацииПоУмолчанию = мПодстрок[мПодстрок.ВГраница()];
		ИмяПубликацииПоУмолчанию = Лев(ИмяПубликацииПоУмолчанию,СтрДлина(ИмяПубликацииПоУмолчанию) - 2);
	Иначе
		//Srvr="***";Ref="***";
		ИмяПубликацииПоУмолчанию = СтроковыеФункцииКлиентСервер.НайтиСтрокуМежду(СтрокаСоединенияИнформационнойБазы,"Ref=""", """;")
	КонецЕсли;
	
	ИмяПубликации = ИмяПубликацииПоУмолчанию;
...
Функция Подключаемый_СтраницаМастераВыборРежимаРаботыПомощника_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если Истина Или ЭтоПереходДалее Тогда
		
		ПолучитьВнешнийIPЭтогоСервера();
		
		WSURLВебСервиса = "http://"+ВнешнийIP+?(ПустаяСтрока(ВнешнийПорт), "", ":"+ВнешнийПорт)+"/"+ИмяПубликации;
		Элементы.ПодсказакаАдресПрограммы.Заголовок = WSURLВебСервиса;
		
		Если ПустаяСтрока(Объект.WSURLВебСервиса) Тогда
			Объект.WSURLВебСервиса = WSURLВебСервиса;
		КонецЕсли;
		Если ПустаяСтрока(Объект.WSИмяПользователя) Тогда
			Объект.WSИмяПользователя = "ws";
			Объект.WSПароль = "ws";
			Объект.WSЗапомнитьПароль = Истина;
		КонецЕсли;
...
&НаКлиенте
Функция Подключаемый_СтраницаМастераОбменСБухгалтерией_ПриПереходеДалее(Отказ)
	
	СтраницаМастераОбменСБухгалтерией_ПриПереходеДалееНаСервере(Отказ)
...
&НаКлиенте
Функция Подключаемый_СтраницаМастераОжиданиеЗавершениеАвтоНажатиеГотово_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	
	СтраницаМастераОжиданиеЗавершениеАвтоНажатиеГотово_ОбработкаДлительнойОперацииНаКлиенте(Отказ);
...
Функция Подключаемый_СтраницаМастераОжиданиеПроверкиПодключенияЧерезВебСервис_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	РасширеннаяПроверка = ОбменДаннымиСервер.ЭтоПодчиненныйУзелРИБ();
	
	ПроверитьПодключениеWSНаКлиенте(Отказ, РасширеннаяПроверка);
	
	ТекстВопроса = "Установить тестовое подключение по заданным параметрам не удалось!
	|Желаете продолжить настройку подключения?";
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Да, "Да, продолжить настройку");
	Кнопки.Добавить(КодВозвратаДиалога.Пропустить, "Нет, пропустить настройку и создать обмен");
	
	Если Отказ И Вопрос(ТекстВопроса, Кнопки, ,	КодВозвратаДиалога.Да, "Во время создания тестового подключения возникли ошибки") = КодВозвратаДиалога.Пропустить Тогда
		Отказ = Ложь;
	КонецЕсли;
...
&НаКлиенте
Функция Подключаемый_СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено_ПриПереходеДалее(Отказ)
	ЗавершениеНастройкиОбменаЧерезВебСервисНаСервере(Объект.УзелИнформационнойБазы, ПредопределенноеРасписаниеОбменаДанными, РасписаниеВыполненияОбменаДанными);
КонецФункции
...
Процедура ПроверитьЗаданиеНастроекДляПервойБазы(Отказ, ТипСоединения = "ВебСервис")
	...
	//.. Начало изменения Dim)on  27 июля 2013 г. 19:05:07
	//
	Если Объект.ИмяПланаОбмена = "ОбменДанными2K_CRS" Тогда
		
		Если ПустаяСтрока(Объект.ПрефиксИнформационнойБазыПриемника) Тогда
			
			НСтрока = НСтр("ru = 'Укажите префикс новой базы.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
			
		КонецЕсли;
		
		Если РесторанВторойБазы.Пустая() Тогда
			
			НСтрока = НСтр("ru = 'Укажите ресторан новой базы.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "РесторанВторойБазы",, Отказ);
			
		Иначе
			ДобавитьДопРеквизит("Ресторан", РесторанВторойБазы);
			                                      
			НастройкиСвязейМеждуБазамиРС = Новый ТаблицаЗначений;
			НастройкиСвязейМеждуБазамиРС.Колонки.Добавить("ИдентификаторОбъектаНастройки");
			НастройкиСвязейМеждуБазамиРС.Колонки.Добавить("ИсточникДанныхОбъекта");
			               
			мИдентификаторовБКиЦРС = Новый Массив;
			
			Для каждого СтрЗагружать Из КонтекстФормыНастройкиУзлов.ЗагружаютсяИЗ_ЦБР Цикл
				ИдентификаторовБКиЦРС = КонтекстФормыНастройкиУзлов.ЗагружаютсяИЗ_БК.Найти(СтрЗагружать.ИдентификаторОбъектаНастройки);
				Если Не ИдентификаторовБКиЦРС = Неопределено Тогда
					мИдентификаторовБКиЦРС.Добавить(СтрЗагружать.ИдентификаторОбъектаНастройки);
					КонтекстФормыНастройкиУзлов.ЗагружаютсяИЗ_БК.Удалить(ИдентификаторовБКиЦРС);
					Продолжить;
				КонецЕсли;
				НСтр = НастройкиСвязейМеждуБазамиРС.Добавить();	
				НСтр.ИдентификаторОбъектаНастройки = СтрЗагружать.ИдентификаторОбъектаНастройки;
				НСтр.ИсточникДанныхОбъекта = Перечисления.ВидыБазИсточниковПриОбменеЦРС.ЦентральнаяБазаРестораннойСистемы;
			КонецЦикла;	
			Для каждого СтрЗагружать Из КонтекстФормыНастройкиУзлов.ЗагружаютсяВ_ЦБР Цикл
				ИдентификаторовБКиЦРС = КонтекстФормыНастройкиУзлов.ЗагружаютсяВ_БК.Найти(СтрЗагружать.ИдентификаторОбъектаНастройки);
				Если Не ИдентификаторовБКиЦРС = Неопределено Тогда
					мИдентификаторовБКиЦРС.Добавить(СтрЗагружать.ИдентификаторОбъектаНастройки);
					КонтекстФормыНастройкиУзлов.ЗагружаютсяВ_БК.Удалить(ИдентификаторовБКиЦРС);
					Продолжить;
				КонецЕсли;
				НСтр = НастройкиСвязейМеждуБазамиРС.Добавить();	
				НСтр.ИдентификаторОбъектаНастройки = СтрЗагружать.ИдентификаторОбъектаНастройки;
				НСтр.ИсточникДанныхОбъекта = Перечисления.ВидыБазИсточниковПриОбменеЦРС.ЦентральнаяБазаРестораннойСистемы;
			КонецЦикла;			
			Для каждого СтрЗагружать Из мИдентификаторовБКиЦРС Цикл
				НСтр = НастройкиСвязейМеждуБазамиРС.Добавить();	
				НСтр.ИдентификаторОбъектаНастройки = СтрЗагружать;
				НСтр.ИсточникДанныхОбъекта = Перечисления.ВидыБазИсточниковПриОбменеЦРС.ЦентральныеБазыРестораннойСистемыИБухгалтерии;
			КонецЦикла;			
			
			Для каждого СтрЗагружать Из КонтекстФормыНастройкиУзлов.ЗагружаютсяИЗ_БК Цикл
				НСтр = НастройкиСвязейМеждуБазамиРС.Добавить();	
				НСтр.ИдентификаторОбъектаНастройки = СтрЗагружать.ИдентификаторОбъектаНастройки;
				НСтр.ИсточникДанныхОбъекта = Перечисления.ВидыБазИсточниковПриОбменеЦРС.ЛокальнаяБазаБухгалтерии;
			КонецЦикла;			
			Для каждого СтрЗагружать Из КонтекстФормыНастройкиУзлов.ЗагружаютсяИЗ_ЛБР Цикл
				НСтр = НастройкиСвязейМеждуБазамиРС.Добавить();	
				НСтр.ИдентификаторОбъектаНастройки = СтрЗагружать.ИдентификаторОбъектаНастройки;
				НСтр.ИсточникДанныхОбъекта = Перечисления.ВидыБазИсточниковПриОбменеЦРС.АвтономнаяРабота;
			КонецЦикла;			
			Для каждого СтрЗагружать Из КонтекстФормыНастройкиУзлов.ЗагружаютсяВ_БК Цикл
				НСтр = НастройкиСвязейМеждуБазамиРС.Добавить();	
				НСтр.ИдентификаторОбъектаНастройки = СтрЗагружать.ИдентификаторОбъектаНастройки;
				НСтр.ИсточникДанныхОбъекта = Перечисления.ВидыБазИсточниковПриОбменеЦРС.ЛокальнаяБазаБухгалтерии;
			КонецЦикла;			
			Для каждого СтрЗагружать Из КонтекстФормыНастройкиУзлов.ЗагружаютсяВ_ЛБР Цикл
				НСтр = НастройкиСвязейМеждуБазамиРС.Добавить();	
				НСтр.ИдентификаторОбъектаНастройки = СтрЗагружать.ИдентификаторОбъектаНастройки;
				НСтр.ИсточникДанныхОбъекта = Перечисления.ВидыБазИсточниковПриОбменеЦРС.АвтономнаяРабота;
			КонецЦикла;			
			ДобавитьДопРеквизит("НастройкиСвязейМеждуБазамиРС", НастройкиСвязейМеждуБазамиРС);
			
		КонецЕсли;
		
	КонецЕсли;
	//
	//.. Конец изменения Dim)on  27 июля 2013 г. 19:05:07
...
&НаКлиенте
Процедура СтраницаМастераОжиданиеЗавершениеАвтоНажатиеГотово_ОбработкаДлительнойОперацииНаКлиенте(Отказ)
...
&НаСервере
Процедура СтраницаМастераОбменСБухгалтерией_ПриПереходеДалееНаСервере(Отказ)
...
//.. Начало изменения Dim)on  27 июля 2013 г. 15:55:53
// упрощенная по сравнению со стандартными
&НаСервере
Процедура ТаблицаПереходовПростойНастройкиОбменаЧерезВебСервис()
	
	ТаблицаПереходов.Очистить();
	
	н = 1;
	//ТаблицаПереходовНоваяСтрока(1,  "СтраницаМастераНачало",, "СтраницаНавигацииНачало");
	ТаблицаПереходовНоваяСтрока(н,  "СтраницаМастераВыборРежимаРаботыПомощника", , "СтраницаНавигацииНачало", , , "СтраницаМастераВыборРежимаРаботыПомощника_ПриОткрытии");
	н = н + 1;
	
	// Задание параметров подключения; Проверка подключения
	ТаблицаПереходовНоваяСтрока(н,  "СтраницаМастераОжиданиеПроверкиПодключенияКСервису", "", "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеПроверкиПодключенияЧерезВебСервис_ОбработкаДлительнойОперации");
	н = н + 1;
	
	// Настройка параметров для выгрузки данных (отборы на узлах)
	ТаблицаПереходовНоваяСтрока(н,  "СтраницаМастераНастройкаПараметровОбменаДаннымиЧерезВебСервис", "", "СтраницаНавигацииПродолжение", "СтраницаМастераНастройкаПараметровОбменаДанными_ПриПереходеДалее",, "СтраницаМастераНастройкаПараметровОбменаДанными_ПриОткрытии");
	н = н + 1;
	
	// Создание настройки обмена; Регистрация справочников для выгрузки
	ТаблицаПереходовНоваяСтрока(н,  "СтраницаМастераОжиданиеАнализаДанныхСозданиеНастройкиОбмена",  "", "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеАнализаДанныхСозданиеНастройкиОбмена_ОбработкаДлительнойОперации");
	н = н + 1;
	...
	//ТаблицаПереходовНоваяСтрока(6, "СтраницаМастераИтогНастройкиОбмена",                    "", "СтраницаНавигацииПродолжение",,,"СтраницаМастераИтогНастройкиОбмена_ПриОткрытии");
	ТаблицаПереходовНоваяСтрока(н, "СтраницаМастераОжиданиеСозданияНастройкиОбменаДанными", "", "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСозданияНастройкиОбменаДанными_ОбработкаДлительнойОперации");
	н = н + 1;
	
	ТаблицаПереходовНоваяСтрока(н, "СтраницаМастераОжиданиеЗавершениеАвтоНажатиеГотово", "", "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеЗавершениеАвтоНажатиеГотово_ОбработкаДлительнойОперации");
	н = н + 1;
	
	ТаблицаПереходовНоваяСтрока(н, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено",, "СтраницаНавигацииПродолжение",,, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено_ПриОткрытии");
	н = н + 1;
	//ТаблицаПереходовНоваяСтрока(6, "СтраницаМастераОкончаниеСВыгрузкойНастройки",           "", "СтраницаНавигацииОкончание");
	
КонецПроцедуры //.. Конец изменения Dim)on  27 июля 2013 г. 15:55:53

&НаСервере
Процедура ТаблицаПереходовПервогоЭтапаНастройкиОбменаСБухгалтерией()
	
	ТаблицаПереходов.Очистить();
	
	ТаблицаПереходовНоваяСтрока(1, "СтраницаМастераНачалоОбменСБухгалтерией",				"", "СтраницаНавигацииНачало", "СтраницаМастераОбменСБухгалтерией_ПриПереходеДалее");
	//ТаблицаПереходовНоваяСтрока(2, "СтраницаМастераИтогНастройкиОбмена",                    "", "СтраницаНавигацииПродолжение",,,"СтраницаМастераИтогНастройкиОбмена_ПриОткрытии");
	ТаблицаПереходовНоваяСтрока(2, "СтраницаМастераОжиданиеСозданияНастройкиОбменаДанными", "", "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСозданияНастройкиОбменаДанными_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(3, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено",	  , "СтраницаНавигацииПродолжение", "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено_ПриПереходеДалее",, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено_ПриОткрытии");
	ТаблицаПереходовНоваяСтрока(4, "СтраницаМастераОкончаниеСВыгрузкойНастройки",           "", "СтраницаНавигацииОкончаниеИНазад");
	
КонецПроцедуры

&НаСервере
Процедура ТаблицаПереходовНастройкаТолькоРасписания()
	
	ТаблицаПереходов.Очистить();
	
	ТаблицаПереходовНоваяСтрока(1, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено",	  , "СтраницаНавигацииОкончание", "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено_ПриПереходеДалее",, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено_ПриОткрытии");
	
КонецПроцедуры

&НаСервере
Процедура ТаблицаПереходовПродолжениеНастройкиОбменаПодчиненногоССервисомГлавного()
	...
	ТаблицаПереходовНоваяСтрока(3, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено",, "СтраницаНавигацииОкончание",,, "СтраницаМастераСозданиеОбменаДаннымиУспешноЗавершено_ПриОткрытии");
...
&НаКлиенте
Процедура ОпределитьПараметрыПодключенияКВебСервисуПриИзменении(Элемент)
	УстановитьВидимостьПанелиНастроекВебСервисаНаСервере();
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьПанелиНастроекВебСервисаНаСервере()

	Элементы.ПараметрыТранспортаWS.Доступность = НастройкаПараметровВручную;	

КонецПроцедуры // УстановитьВидимостьПанелиНастроекВебСервисаНаСервере()

&НаСервере
Процедура ДобавитьДопРеквизит(Ключ, Значение)

	Если КонтекстФормыНастройкиУзлов.Свойство("СтруктураДопРеквизитов") Тогда
		СтруктураДопРеквизитов = КонтекстФормыНастройкиУзлов.СтруктураДопРеквизитов;
	Иначе
		СтруктураДопРеквизитов = Новый Структура;
	КонецЕсли;
	
    СтруктураДопРеквизитов.Вставить(Ключ, Значение);
	
	КонтекстФормыНастройкиУзлов.Вставить("СтруктураДопРеквизитов", СтруктураДопРеквизитов);
...
Функция ПолучитьНовыйПрефиксРесторана()
	...
	Возврат "R" + Формат(Индекс + 1, "ЧЦ=2; ЧВН="); 	

КонецФункции

&НаКлиенте
Процедура РесторанВторойБазыПриИзменении(Элемент)
	Если Не Объект.НаименованиеВторойБазы = Строка(РесторанВторойБазы) Тогда //ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		Объект.НаименованиеВторойБазы = Строка(РесторанВторойБазы)	
	КонецЕсли;
	Если Не Объект.ПрефиксИнформационнойБазыПриемникаУстановлен Тогда
		//Объект.ПрефиксИнформационнойБазыПриемника = ПолучитьНовыйПрефиксРесторана()
		//Костенюк Александр-Старт 03.06.2014
		// Установка префикса периферийного узла
		Если ЗначениеЗаполнено(РесторанВторойБазы.Префикс) Тогда
			Объект.ПрефиксИнформационнойБазыПриемника = СокрЛП(РесторанВторойБазы.Префикс);
		Иначе
			Объект.ПрефиксИнформационнойБазыПриемника = ПолучитьНовыйПрефиксРесторана();
		КонецЕсли;
...
&НаКлиенте
Процедура НастроитьДополнительныеПараметрыFTPПриИзменении(Элемент)
	Элементы.ГруппаДополнительно.Видимость = НастроитьДополнительныеПараметрыFTP
КонецПроцедуры

&НаКлиенте
Процедура ВидТранспортаСообщенийОбменаСБухгалтериейПриИзменении(Элемент)
	
	СтраницыПараметровТранспорта = Новый Структура;
	СтраницыПараметровТранспорта.Вставить("СетевойКаталог", Элементы.НастройкиТранспортаFILE1);
	СтраницыПараметровТранспорта.Вставить("FTP",          Элементы.НастройкиТранспортаFTP1);
	                                         
	Элементы.СтраницыВидовТранспорта.ТекущаяСтраница = СтраницыПараметровТранспорта[ВидТранспортаСообщенийОбменаСБухгалтерией];
	
КонецПроцедуры

&НаКлиенте
Процедура ИмяПубликацииПриИзменении(Элемент)
	СформироватьАдресВебСервиса()
КонецПроцедуры

&НаКлиенте
Процедура ВнешнийIPПриИзменении(Элемент)
	СформироватьАдресВебСервиса()
КонецПроцедуры

&НаКлиенте
Процедура ВнешнийПортПриИзменении(Элемент)
	СформироватьАдресВебСервиса()
КонецПроцедуры

&НаКлиенте
Процедура СформироватьАдресВебСервиса()

	WSURLВебСервиса = "http://"+ВнешнийIP+?(ПустаяСтрока(ВнешнийПорт), "", ":"+ВнешнийПорт)+"/"+ИмяПубликации;
	
	Если Не WSURLВебСервиса = Объект.WSURLВебСервиса Тогда
		Объект.WSURLВебСервиса = WSURLВебСервиса;
	КонецЕсли;
	Если ПустаяСтрока(Объект.WSИмяПользователя) Тогда
		//Объект.WSИмяПользователя = "WS2K"; //ИмяПользователя();
		//Объект.WSПароль = "exchange";
		//Костенюк Александр-Старт 04.04.2014
		Объект.WSИмяПользователя = "ws"; //ИмяПользователя();
		Объект.WSПароль = "ws";
		//Костенюк Александр-Финиш 04.04.2014
		Объект.WSЗапомнитьПароль = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура FILEСжиматьФайлИсходящегоСообщения1ПриИзменении(Элемент)
	Элементы.ПарольАрхиваСообщенияОбмена3.Доступность = Объект.FILEСжиматьФайлИсходящегоСообщения;
КонецПроцедуры

&НаКлиенте
Процедура FTPСжиматьФайлИсходящегоСообщения1ПриИзменении(Элемент)
	Элементы.ПарольАрхиваСообщенияОбмена4.Доступность = Объект.FTPСжиматьФайлИсходящегоСообщения;
КонецПроцедуры
...
Функция GetIPAdress()
	...
	Попытка
		...
		ОписаниеСервиса = Новый  WSОпределения("http://develplatform.somee.com/SynIP.asmx?wsdl");
