...
//
// Параметры:
//   ИмяРаздела (Строка) Имя раздела, для которого открывается панель отчетов.
...
//
Процедура ПоказатьПанельОтчетов(ИмяРаздела, ПараметрыВыполненияКоманды, Заголовок = "") Экспорт
	ФормаПараметры = Новый Структура("ИмяРаздела", ИмяРаздела);
	Если ЗначениеЗаполнено(Заголовок) Тогда
		ФормаПараметры.Вставить("Заголовок", Заголовок);
	КонецЕсли;
	
	ФормаВладелец     = ПараметрыВыполненияКоманды.Источник;
	ФормаУникальность = ИмяРаздела + "." + ПараметрыВыполненияКоманды.Уникальность;
...
КонецПроцедуры // ОткрытьПанельОтчетов
...
//
Процедура ОткрытьДиалогРамещенияВариантовВРазделах(МассивВариантов, ДополнительныеПараметры = Неопределено) Экспорт
...
КонецПроцедуры // ОткрытьДиалогРамещенияВариантовВРазделах 

// Обновляет открытые панели отчетов.
//
// Параметры:
//  см. справку к Оповестить (Глобальный контекст)
//
Процедура ОбновитьОткрытыеПанелиОтчетов(Параметр = Неопределено, Источник = Неопределено) Экспорт
	
	Оповестить(ВариантыОтчетовКлиентСервер.ИмяСобытияИзменениеВарианта(), Параметр, Источник);
	
КонецПроцедуры // ОбновитьОткрытыеПанелиОтчетов
...
Процедура ОткрытьВариантОтчета(ЭтаФорма) Экспорт
	...
	Иначе
		...
		ПараметрыОткрытия = Новый Структура("КлючВарианта", Вариант.КлючВарианта);
		ОткрытьФорму("Отчет." + ИмяОтчета + ".Форма", ПараметрыОткрытия);
